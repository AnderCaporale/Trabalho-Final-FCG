...
int main()
{
    ...
    GLFWwindow* window;
    window = glfwCreateWindow(800, 800, "INF01047 - Trabalho Final FCG", NULL, NULL);
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (long long unsigned int i = 0; i<paredes.size(); ++i)
        {
            ...
            if ( paredes[i].dir == 1 ){
                ...
                model = Matrix_Translate(paredes[i].x + 0.495f, paredes[i].y, -paredes[i].z) *
                        Matrix_Scale(0.01f, 1.0f, 1.00f);
                glUniformMatrix4fv(model_uniform, 1, GL_FALSE, glm::value_ptr(model));
                DrawCube(render_as_black_uniform);

            } else if ( paredes[i].dir == 0 ){

                model = Matrix_Translate(paredes[i].x, paredes[i].y, -paredes[i].z - 0.495f )*
